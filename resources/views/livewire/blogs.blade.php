						...
						<div class="col-md-6" data-aos="fade">
							<article class="blog-post">
								<div class="post-slider slider-sm rounded">
									<img loading="lazy" decoding="async" src="{{asset('storage/'.$article->image)}}" class="img-fluid" alt="{{$article->image}}" alt="Post Thumbnail">
									
								</div>
								<div class="pt-4">
									<p class="mb-3">{{ date('M j, Y', strtotime($article->created_at)) }}</p>
									<h2 class="h4"><a wire:navigate class="text-black" href="{{route('blogdetail', $article->id)}}">{{$article->title}}</a></h2>
									<p>{{ substr(strip_tags($article->content), 0, 500) }}</p> <a href="#" class="text-primary fw-bold" aria-label="Read the full article by clicking here">{{ strlen(strip_tags($article->body)) > 50 ? "...ReadMore" : "" }}</a>
								</div>
							</article>
						</div>
						@empty
			...
			<div class="col-lg-3">
				...
				<div class="widget widget-categories">
					...
					<ul class="list-unstyled widget-list">
						...
						<li><a wire:navigate href="{{route('blogs').'?&categorySlug='.$category->slug}}">{{$category->name}} <small class="ml-auto">(0)</small></a>
						</li>
                        @empty
                    ...
                    <ul class="list-unstyled widget-list">
						<li class="d-flex widget-post align-items-center">
							...
							<div class="flex-grow-1">
								...
								<small>{{ date('M j, Y', strtotime($latestArticle->created_at)) }}</small>
							</div>
						</li>
					</ul>
                    @empty
